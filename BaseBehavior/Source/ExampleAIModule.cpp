...
//	Broodwar->sendText("frames: %i frameScale: %i TimeToBuildFirtsBuilding: %i",Broodwar->getFrameCount(),Broodwar->getFrameCount()%2160,TimeToBuildFirtBuilding);
	...
	{
		if((*i)->getType().isResourceDepot())
		{ // if the unit is a resource depot (begin)
			if(Broodwar->self()->getRace() == Races::Zerg)
			{
				std::set<Unit *> larva = (*i)->getLarva();
				if(larva.size() > 0)
				{
					bool worker = false;
					for(std::set<Unit *>::const_iterator l = larva.begin();l!=larva.end();l++)
					{
						if(!(*l)->isMorphing())
						{
							if(Broodwar->self()->supplyUsed() + 4 >= Broodwar->self()->supplyTotal())
							{
								if(MineralsSpent + UnitTypes::Zerg_Overlord.mineralPrice() <= Broodwar->self()->minerals())
								{
									if((*l)->morph(Broodwar->self()->getRace().getSupplyProvider()))
									{
										MineralsSpent += Broodwar->self()->getRace().getSupplyProvider().mineralPrice();
										continue;
									}
								}
							}
							else if(MineralsSpent + UnitTypes::Zerg_Drone.mineralPrice() <= Broodwar->self()->minerals() && !worker)
							{
								if((*l)->morph(Broodwar->self()->getRace().getWorker()))
								{
									MineralsSpent += Broodwar->self()->getRace().getWorker().mineralPrice();
									worker = true;
								}
							}
							else if(Broodwar->self()->allUnitCount(UnitTypes::Zerg_Spawning_Pool) == 1)
							{
								if(MineralsSpent + UnitTypes::Zerg_Zergling.mineralPrice() <= Broodwar->self()->minerals())
								{
									if((*l)->morph(UnitTypes::Zerg_Zergling))
									{
										MineralsSpent += UnitTypes::Zerg_Zergling.mineralPrice();
									}
								}
							}
						}
			...
			{
				if((*i)->isIdle() && MineralsSpent + Broodwar->self()->getRace().getWorker().mineralPrice() <= Broodwar->self()->minerals()
					&& GasSpent + Broodwar->self()->getRace().getWorker().gasPrice() <= Broodwar->self()->gas())
				{
		...
		} // if the unit is a resource depot (end)
		else if((*i)->getType().isWorker())
		{ // if the unit is a worker (begin)
			if((*i)->isIdle())
			{
			...
			if(Broodwar->self()->supplyUsed() + 4 >= Broodwar->self()->supplyTotal() && Broodwar->self()->getRace() != Races::Zerg) // we need more supply Sources
			{
				if(MineralsSpent + Broodwar->self()->getRace().getSupplyProvider().mineralPrice() <= Broodwar->self()->minerals()
					&& GasSpent + Broodwar->self()->getRace().getSupplyProvider().gasPrice() <= Broodwar->self()->gas())
				{        // if we have enought minerals and gas to build
					//Broodwar->sendText("We have minerals!");
					if(!IsOnBuildingList(BController,Broodwar->self()->getRace().getSupplyProvider())) // if this building isn't in the list
					{
						Broodwar->sendText("It is not on the list!");
						TilePosition BuildingPosition = GetValidBuildingLocation(BsManager.Bases->baseTilePosition,Broodwar->self()->getRace().getSupplyProvider(),(*i));
						if(BuildingPosition != TilePosition(0,0)) // if we found an valid tileposition
						{
							Broodwar->sendText("Add it to the list!");
							AddBuildingInList(&BController,BuildingPosition,Broodwar->self()->getRace().getSupplyProvider(),&MineralsSpent,&GasSpent,true);
						}
					}
				}
			}
			if(Broodwar->self()->allUnitCount(UnitTypes::Protoss_Gateway) <= 5 || Broodwar->self()->allUnitCount(UnitTypes::Terran_Barracks) <= 5 ||
				Broodwar->self()->allUnitCount(UnitTypes::Zerg_Spawning_Pool) < 1)
			{
				if(Broodwar->self()->getRace() == Races::Protoss && Broodwar->getFrameCount() >= TimeToBuildFirtBuilding)
				{
					if(UnitTypes::Protoss_Gateway.mineralPrice() + MineralsSpent >= Broodwar->self()->minerals() &&
						UnitTypes::Protoss_Gateway.gasPrice() + GasSpent >= Broodwar->self()->gas())
					{
						TilePosition BuildingPosition = GetValidBuildingLocation(GetPositionPylon((*i)),UnitTypes::Protoss_Gateway,(*i));
						if(BuildingPosition != TilePosition(0,0) && !IsOnBuildingList(BController,UnitTypes::Protoss_Gateway))
						{
							Broodwar->sendText("Protoss Gateway was added to the list");
							AddBuildingInList(&BController,BuildingPosition,UnitTypes::Protoss_Gateway,&MineralsSpent,&GasSpent,false);
							TimeToBuildFirtBuilding += 2160;
						}
					}
				}
				else if(Broodwar->self()->getRace() == Races::Terran && Broodwar->getFrameCount() >= TimeToBuildFirtBuilding)
				{
					if(UnitTypes::Terran_Barracks.mineralPrice() + MineralsSpent >= Broodwar->self()->minerals() &&
						UnitTypes::Terran_Barracks.gasPrice() + GasSpent >= Broodwar->self()->gas())
					{
						Broodwar->sendText("Time To Build Barracks!");
						TilePosition BuildingPosition = GetValidBuildingLocation(BsManager.Bases->baseTilePosition,UnitTypes::Terran_Barracks,(*i));
						Broodwar->drawCircleMap(BuildingPosition.x()*32,BuildingPosition.y()*32,100,Colors::Red,false);
						if(BuildingPosition != TilePosition(0,0) && !IsOnBuildingList(BController,UnitTypes::Terran_Barracks))
						{
							Broodwar->sendText("Terran Barracks was added to the list");
							AddBuildingInList(&BController,BuildingPosition,UnitTypes::Terran_Barracks,&MineralsSpent,&GasSpent,false);
							TimeToBuildFirtBuilding += 2160;
						}
					}
				}
				else if(Broodwar->self()->getRace() == Races::Zerg && Broodwar->getFrameCount() >= TimeToBuildFirtBuilding)
				{
					if(UnitTypes::Zerg_Spawning_Pool.mineralPrice() + MineralsSpent >= Broodwar->self()->minerals() &&
						UnitTypes::Zerg_Spawning_Pool.gasPrice() + GasSpent >= Broodwar->self()->gas())
					{
						TilePosition BuildingPosition = GetValidBuildingLocation(BsManager.Bases->baseTilePosition,UnitTypes::Zerg_Spawning_Pool,(*i));
						if(BuildingPosition != TilePosition(0,0) && !IsOnBuildingList(BController,UnitTypes::Zerg_Spawning_Pool))
						{
							Broodwar->sendText("Zerg Spawning Pool was added to the list");
							AddBuildingInList(&BController,BuildingPosition,UnitTypes::Zerg_Spawning_Pool,&MineralsSpent,&GasSpent,false);
							TimeToBuildFirtBuilding += 2160;
						}
					}
				}
			...
			{
				BuildFirstInList(&BController,(*i),&MineralsSpent,&GasSpent);
			}
		} // if the unit is a worker (end)
		else if((*i)->getType() == UnitTypes::Protoss_Gateway || (*i)->getType() == UnitTypes::Terran_Barracks)
		{
			if((*i)->isIdle())
			{
				if(Broodwar->self()->getRace() == Races::Protoss)
				{
					(*i)->train(UnitTypes::Protoss_Zealot);
				}
